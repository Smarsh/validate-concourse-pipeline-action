...
while IFS="," read -r name file; do
    if [ ! -f "${file}" ] && [[ ${file} != interpolated-versions* ]] && [[ ${file} != versions-ui_portal_* ]] && [[ ${file} != "parsed-version/version.yml" ]] && [[ ! (${file} =~ event-logging-git/*) ]]  && [[ ! (${file} =~ insights-jobs-app-git/*) ]] && [[ ${file} != versions-*/.ref ]] && [[ ${file} != *app*/version ]] && [[ ${file} != "artifact-version/version" ]]; then
