...
while IFS="," read -r name file; do
    if [ ! -f "${file}" ] && [[ ${file} != interpolated-versions* ]] && [[ ${file} != versions-ui_portal_* ]] && [[ ! (${file} =~ ea-hazelcast-deployment-src/*) ]]  && [[ ! (${file} =~ event-logging-git/*) ]]  && [[ ! (${file} =~ deployment-artifact/*) ]] && [[ ! (${file} =~ insights-jobs-app-git/*) ]]  && [[ ! (${file} =~ git-properties-*/*) ]] && [[ ! (${file} =~ ea-policy-evaluation-service-git/*) ]]  && [[ ! (${file} =~ ea-ediscovery-api-git/*) ]]  && [[ ${file} != versions-*/.ref ]] && [[ ${file} != */version ]] && [[ ${file} != */commitsha1 ]] && [[ ${file} != ipList/ipList.txt ]] && [[ ! (${file} =~ ea-audio-data-service-ci-source/*) ]] && [[ ! (${file} =~ ci-audio-comms-ci-source/*) ]] && [[ ${file} != platform-automation-tasks/tasks/credhub-interpolate.yml ]] && [[ ! (${file} =~ ea-ui-portal/*) ]] && [[ ! (${file} =~ smoke-tests/*) ]]; then
